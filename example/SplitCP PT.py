...
if __name__=="__main__":
    ...
    for i in range(0, T):
        ...
        Split_CP_PT = Conformal(X0=X0, y0=y0, X00=X00, y00=y00, rho=0.5, alpha=0.1, weight=None, train_model="lasso")
        cov[i] = np.mean(Split_CP_PT.split_weight_conformal()[0])
        length[i] = np.median(Split_CP_PT.split_weight_conformal()[1])
